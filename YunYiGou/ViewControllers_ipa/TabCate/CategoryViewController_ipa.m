...
@interface CateViewController_ipa () <UISearchBarDelegate,QRCodeReaderDelegate,UIAlertViewDelegate>
{
    int     _iTopBarH;
    int     _iTVCellH;
    bool    _bIsRequested;
...
@implementation CateViewController_ipa
...
- (void) reloadScrollView : (NSInteger) indexSel
{
    ...
    CGPoint startP = CGPointMake(70, 60);       // 起始坐标
    int distanceX = 110;                         // 横向距离间隔
    int distanceY = 115;
    ...
    for ( int idx = 0; idx < [childrenCate count]; idx++ )
    {
        ...
        centerP.x = startP.x + idx%5*distanceX;
        centerP.y = startP.y + idx/5*distanceY;
        
        // 分类图片
        UIImageView * cateIV = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 70, 70)];
        ...
        // 分类名称
        UILabel* nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 60, 15)];
        [nameLabel setText:cateDic[@"name"]];
        [nameLabel setCenter:CGPointMake(centerP.x, centerP.y+50)];
        [nameLabel setNumberOfLines:1];
    ...
    [_cate2ndScrollView setContentSize:CGSizeMake(self.view.frame.size.width*4/5, startP.y+[childrenCate count]/5*distanceY+distanceY/2)];
}
