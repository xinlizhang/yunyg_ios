...
#import "ArticleDetailViewController_ipa.h"
#import "RDVTabBarController.h"
#import "UIImageView+OnlineImage.h"
#import "MyAlert.h"

@interface ArticleDetailViewController_ipa ()
{
...
@implementation ArticleDetailViewController_ipa
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self requestData:self.strArticleID2Info];
...
-(void)requestData : (NSString*)articleID
{
    NetworkModel * net = [NetworkModel sharedNetworkModel];
    
        [net request_Article_Datasuc:^(id success) {
            if (success[@"status"])
            {
