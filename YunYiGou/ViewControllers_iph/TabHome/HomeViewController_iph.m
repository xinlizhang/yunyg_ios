...
#import "TopOfHomeScrollView_iph.h"
...
@interface HomeViewController_iph ()<EGORefreshTableDelegate,UISearchBarDelegate,QRCodeReaderDelegate,UIAlertViewDelegate,UIScrollViewDelegate,TopOfHomeScrollViewProtocol_iph, LimitPromotionViewProtocol_iph, HotGoodsViewProtocol_iph,RecommendViewProtocol_iph, NewArrivalViewProtocal_iph, CategoryViewProtocal_iph>
{
    ...
    TopOfHomeScrollView_iph * _topScrollView;//头条 横向滚动
    LimitPromotionView_iph * _limitPromotionView;
    HotGoodsView_iph * _hotGoodView;
    RecommendView_iph * _recommendView;
    NewArrivalView_iph * _newArriavalView;
    CategoryView_iph * _categoryView;
...
@implementation HomeViewController_iph
...
-(void)initUI
{
    ...
    UIButton * scanningBtn = [[UIButton alloc] initWithFrame:CGRectMake(8, 28, 25, 25)];
    ...
    UIButton * voiceBtn = [[UIButton alloc] initWithFrame:CGRectMake(r.size.width - 8 - 25, 28, 25, 25)];
    ...
    // 搜索按钮
    _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(35, 25, r.size.width - 35*2, 33)];
    ...
    top = 0;
    height = 180;
    _topScrollView = [[TopOfHomeScrollView_iph alloc] initWithFrame:CGRectMake(0, top, r.size.width, height)];
    ...
    if (_hasLimitPromotion) {
        ...
        _limitPromotionView = [[LimitPromotionView_iph alloc] initWithFrame:CGRectMake(0, top, r.size.width, height)];
    ...
    if (_hasHotGoods) {
        ...
        _hotGoodView = [[HotGoodsView_iph alloc] initWithFrame:CGRectMake(0, top, r.size.width, height)];
    ...
    if (_hasRecommend) {
        ...
        _recommendView = [[RecommendView_iph alloc] initWithFrame:CGRectMake(0, top, r.size.width, height)];
    ...
    if (_hasNewArrival) {
        ...
        _newArriavalView = [[NewArrivalView_iph alloc] initWithFrame:CGRectMake(0, top, r.size.width, height)];
    ...
    if (_hasCotecary) {   
        top += height + 10;
        height = 300;
        _categoryView = [[CategoryView_iph alloc] initWithFrame:CGRectMake(0, top, r.size.width, height)];
...
-(void)requestCheckUpdate
{
    ...
    [net request_CheckUpdate_Datasuc:^(id success) {
        if ( success[@"status"] && [[success[@"status"] valueForKey:@"succeed"] intValue] == 1 )
        {
            ...
            {
                _updateURL = [[NSString alloc] initWithString:[dic valueForKey:@"URL_Iphone"]];
