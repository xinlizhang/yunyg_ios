...
@interface CartViewController_iph () <EGORefreshTableDelegate>
{
    int     _iTopBarH;
    int     _iTVCellH;
    int     _iTVCellTotalH;
...
@implementation CartViewController_iph
...
-(void)initUI
{
    ...
    _cartTableView = [[[UITableView alloc] initWithFrame:CGRectMake((frameR.size.width-tableviewW)/2, _iTopBarH, tableviewW, frameR.size.height-_iTopBarH-50) style:UITableViewStylePlain] autorelease];
    ...
    _refreshHeaderView = [[EGORefreshTableHeaderView alloc] initWithFrame:CGRectMake(0.0f, 0.0f - _cartTableView.bounds.size.height, frameR.size.width, _cartTableView.bounds.size.height)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( [_cartDataArray count] == indexPath.row )
    {
        ...
        CGPoint relationP = CGPointMake(30, 10);
        // 总计文字
    ...
    // 商品图片
    CGPoint relationP = CGPointMake(13, 8);
    ...
    // 横向分割线
    UIImageView* seperatorlINE = [[UIImageView alloc] initWithFrame:CGRectMake(5, _iTVCellH, cellR.size.width-10, 1)];
...
{
    NSLog(@"GoToPay!");
    BasicViewController_iph *viewControllerList = [[OrderCheckViewController_iph alloc] init];
    [self.navigationController pushViewController:viewControllerList animated:YES];
}
